...
/* ----------------------------
   ...
   Why: Your dropdown was empty because SportEntity.filter likely isn't the method
   in your current SDK build, and errors were swallowed.
----------------------------- */
async function entityList(Entity, whereObj) {
  if (!Entity) throw new Error("Entity is null/undefined.");
  const where = whereObj || {};

  // Most common in Base44 projects
  ...
  if (typeof Entity.list === "function") {
    // Some SDKs accept { where }, some accept where directly; try both
    try {
...
const SPORTSUSA_DIRECTORY_BY_SPORTNAME = {
  ...
  // ✅ FIX: support both variants seen in earlier workstreams
...
export default function AdminImport() {
  ...
  // ✅ NEW: Camp entity guard (prevents missing export from breaking the page)
  ...
  /* ----------------------------
     ✅ UPDATED: Load Sports (top selector)
  ...
  async function loadSports() {
    ...
    try {
      ...
      // Helpful breadcrumb for debugging “empty dropdown”
  ...
  /* ----------------------------
     ...
     ✅ UPDATED: guard if Camp entity missing
  ----------------------------- */
