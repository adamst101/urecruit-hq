...
function buildSchoolIndex(schools) {
  ...
  for (var i = 0; i < schools.length; i++) {
    ...
    if (nn) {
      ...
      // Also index by fully-normalized form (strips unicode dashes + punctuation)
      var nnStripped = normalizeName(nn);
      if (nnStripped && nnStripped !== nn) {
        if (!byNormName[nnStripped]) byNormName[nnStripped] = [];
        var alreadyIn = byNormName[nnStripped].some(function(e) { return e.id === sid; });
        if (!alreadyIn) byNormName[nnStripped].push({ id: sid, school: s });
      }
    }
