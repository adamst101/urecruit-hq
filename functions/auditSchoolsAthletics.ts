...
// Synced from Base44 AI — all success criteria passing.
//
...
//   "update" — writes division/conference/nickname/wikipedia_url to confirmed athletics schools
//   "delete" — deletes rows flagged as non-athletics (use after reviewing audit report)
//
// { "mode": "audit",  "maxRows": 100, "startAt": 0, "sleepMs": 400 }
// { "mode": "update", "maxRows": 50,  "startAt": 0, "sleepMs": 400, "dryRun": false }
// { "mode": "delete", "maxRows": 200, "startAt": 0, "sleepMs": 200, "dryRun": true }
...
function extractAthleticsFromHtml(html: string, wikipediaUrl: string): AthleticsInfo {
  ...
  function matchedNearAthletics(text: string, pattern: RegExp): boolean {
    ...
    while ((m = re.exec(text)) !== null) {
      ...
      if (ATHLETICS_ANCHORS.test(text.slice(start, end))) return true;
    }
    return false;
  }
  ...
  for (const { pattern, division } of DIVISION_PATTERNS) {
    if (matchedNearAthletics(stripped, pattern)) {
      result.division = division; result.hasAthletics = true; result.confidence = "high";
  ...
  if (!result.division) {
    for (const { pattern, division } of DIVISION_PATTERNS) {
      if (matchedNearAthletics(full, pattern)) {
        result.division = division; result.hasAthletics = true; result.confidence = "medium";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const skipConfirmed = body?.skipConfirmed !== false;
    ...
    const stats: Record<string, any> = {
      mode, dryRun, startAt,
      scanned: 0, alreadyConfirmed: 0, wikipediaFetched: 0,
      athleticsFound: 0, noAthleticsFound: 0, wikiNotFound: 0,
      updated: 0, deleted: 0, errors: 0,
      nextStartAt: 0, done: false, elapsedMs: 0,
    };

    const confirmed: any[] = [], athleticsFound: any[] = [];
    const flaggedForDeletion: any[] = [], wikiNotFound: any[] = [];
    ...
    for (const row of slice) {
      ...
      if (info.hasAthletics) {
        ...
        athleticsFound.push({
          schoolId, school_name: schoolName,
          division: info.division, conference: info.conference,
          nickname: info.nickname, confidence: info.confidence,
          wikipediaUrl: wikiUrl, matchedPatterns: info.matchedPatterns,
        });
